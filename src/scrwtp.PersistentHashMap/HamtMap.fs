...
module HamtMap =
    ...
    module internal Prelude = 
        ...
        module Option = 
            let maybe (def: 'b) (f: 'a -> 'b) (opt: 'a option) = 
                match opt with
                | Some v -> f v
                | None -> def
    ...
    module Hashing = 
        ...
        let hash : 'a -> Hash = Operators.hash

        let fragment shift (h : Hash) : Hash = 
            (h >>> shift) &&& 0b11111
    ...
    module Binary =
        ...
        let toBitmap (subHash: Hash) = 
            1 <<< subHash

        let fromBitmap (bitmap: Bitmap) (subHash: Hash) = 
